...
When(/^I click facebook image$/, () => {
	...
	browser.switchWindow('https://www.facebook.com/login.php?skip_api_login=1&api_key=966242223397117&signed_next=1&next=https%3A%2F%2Fwww.facebook.com%2Fsharer%2Fsharer.php%3Fu%3Dhttps%253A%252F%252Fstaging.diamondere.com%252Fp%252Frings-sidestone-xylos-10847-10676%253Fmetal%253D14k-white-gold%26stones%3Dsi-diamond%26t%3DXylos%2BRing%2Bwith%2BRound%2BRuby%252C%2BSI%2BDiamond%2B%257C%2B0.77%2Bcarats%2BRound%2BRuby%2BSidestone%2BRing%2Bin%2B14k%2BWhite%2BGold%2B%257C%2BDiamondere&cancel_url=https%3A%2F%2Fwww.facebook.com%2Fdialog%2Fclose_window%2F%3Fapp_id%3D966242223397117%26connect%3D0%23_%3D_&display=popup&locale=en_GB')
});
...
When(/^I click select  his ringsize dropdown$/, () => {
	const ringsizearea = new productdetailspage;
	ringsizearea.clickhisringselect();
});
...
Then(/^I ensure page redirects to facebook$/, async() => {
	const facebbookimage = await browser.getUrl()
...
Then(/^I ensure matching band product image,name, price, product description in full details popup is displayed$/, async() => {
	...
	await expect(matchingbandpopuparea.popupMatchingBandPrice).toExist();
...
Then(/^I ensure clicking product from recently viewed redirects to page-3$/, async() => {
	...
	await expect(recentlyviewedarea).toEqual(data.Recentlyviewed['page-3url']);
});
...
Then(/^I ensure rainbow sapphire change stone dropdown is displayed$/, async() => {
	...
	await expect(rainaccentstonearea.lnkaccentstone1dropdown).toExist();
});
...
Then(/^I ensure coupleband product price is changed$/, async() => {
	...
	await expect(changedcoupletextarea).toEqual(data.Price.changedcoupleherpricebyringsize)
});
...
Then(/^I ensure push back and screw back price is same$/, async() => {
	...
	await expect(changedearringprice).not.toEqual(data.Price.changedpricebyearringback)
