...
class writereview{
    ...
    get fourstarrating(){
        return $('//span[@aria-label="Rating-4 of 5"]')
    }
    ...
    enterReviewtitle(){
        ElementUtil.sendText(this.boxtitleRevForm,data.WriteReview['Review Title'],"Send review title in respective field")
    }

    enterReviewcomments(){
        ElementUtil.sendText(this.boxurRevCommnts,data.WriteReview['Review Comments'],"Send review comments in respective field")
    }
    ...
    selectSubmitbutton(){
        ElementUtil.click(this.btnSbmtRev,"Click on Submit button")
    }

    enterinvalidnamereview(){
        ElementUtil.sendText(this.boxnameRevForm,data.WriteReview['Invalid Name'],"Send invalid name in name field")
    }

    enterinvalidemailIdreview(){
        ElementUtil.sendText(this.boxemailRevForm,data.WriteReview['Invalid EmailId'],"Send invalid emailId in emailId field")
    }

    enterinvalidlocationreview(){
        ElementUtil.sendText(this.boxlocationRevForm,data.WriteReview['Invalid Location'],"Send Invalid Location in Location field")
    }

    enterinvalidReviewtitle(){
        ElementUtil.sendText(this.boxtitleRevForm,data.WriteReview['Invalid Review Title'],"Send Invalid review title in respective field")
    }

    enterinvalidReviewcomments(){
        ElementUtil.sendText(this.boxurRevCommnts,data.WriteReview['Invalid Review Comments'],"Send invalid review comments in respective field")
    }
